# Hi, I'm Faisul Alam 👋

### Performance Marketing & Web Analytics Specialist

I help businesses turn advertising data into measurable growth through
paid media, conversion tracking, analytics, and optimization.

---

## 🚀 What I Do

- 📈 Google Ads
- 📣 Meta Ads
- 📊 GA4 & Web Analytics
- 🏷️ Google Tag Manager (GTM)
- 🎯 Conversion Tracking
- 🔵 Meta Pixel & Conversions API
- 🔎 SEO & Search Console
- 🛒 E-commerce Analytics
- 📊 Looker Studio
- 🤖 Marketing Automation & AI

---

## 🧠 Technical Focus

**Paid Media**
- Google Ads
- Meta Ads
- PPC Campaign Strategy
- Audience Research
- Conversion Optimization
- Remarketing

**Analytics & Tracking**
- GA4
- Google Tag Manager
- Meta Pixel
- Meta Conversions API
- Google Ads Conversion Tracking
- Event Tracking
- UTM & Attribution
- Enhanced Conversions

**Data & Reporting**
- Looker Studio
- Google Sheets
- Excel
- Marketing Dashboards
- KPI Analysis

**Web & SEO**
- WordPress
- Shopify
- Google Search Console
- Google Merchant Center
- SEO
- CRO

---

## 🛠️ Tools & Technologies

![Google Ads](https://img.shields.io/badge/Google%20Ads-4285F4?style=for-the-badge&logo=googleads&logoColor=white)
![Meta Ads](https://img.shields.io/badge/Meta%20Ads-0866FF?style=for-the-badge&logo=meta&logoColor=white)
![GA4](https://img.shields.io/badge/GA4-E37400?style=for-the-badge&logo=googleanalytics&logoColor=white)
![GTM](https://img.shields.io/badge/Google%20Tag%20Manager-246FDB?style=for-the-badge&logo=googletagmanager&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![WordPress](https://img.shields.io/badge/WordPress-21759B?style=for-the-badge&logo=wordpress&logoColor=white)
![Shopify](https://img.shields.io/badge/Shopify-7AB55C?style=for-the-badge&logo=shopify&logoColor=white)
![Google Sheets](https://img.shields.io/badge/Google%20Sheets-34A853?style=for-the-badge&logo=googlesheets&logoColor=white)

---

## 📂 Featured Projects

### 📊 GA4 + GTM E-commerce Tracking
Implementation of an e-commerce measurement framework using GA4
and Google Tag Manager.

**Focus:** Event Tracking • Data Layer • GA4 • GTM • Debugging

---

### 🎯 Meta Pixel + CAPI Tracking
Server-side and browser-side tracking implementation for improved
conversion measurement.

**Focus:** Meta Pixel • CAPI • Event Deduplication • EMQ

---

### 📈 Google Ads Conversion Tracking
Conversion tracking framework for Google Ads using GTM and GA4.

**Focus:** Google Ads • GA4 • GTM • Conversion Tracking

---

### 📊 Marketing Analytics Dashboard
Performance reporting dashboard for paid media and website analytics.

**Focus:** Looker Studio • GA4 • Google Ads • KPI Analysis

---

## 📚 Currently Learning

- Advanced Web Analytics
- Server-Side Tracking
- Attribution & Incrementality
- Marketing Automation
- AI for Performance Marketing
- Advanced Google & Meta Ads

---

## 📫 Connect With Me

- 💼 LinkedIn: [Faisul Alam](https://www.linkedin.com/in/faisul-alam/)
- 🌐 Portfolio: [My Website](https://faisulalam.lovable.app/)
- 📧 Email: faisulalam61@gmail.com

---

### 📌 My Approach

**TRACK → MEASURE → ANALYZE → OPTIMIZE → GROW**

> Better data leads to better marketing decisions.
